...
//import ActionLogRepository;
import com.mahadi.restapi.dto.JsonDiffDto;
import com.mahadi.restapi.dto.Response;
import com.mahadi.restapi.model.BaseModel;
...
import com.mahadi.restapi.util.JsonDiffUtil;
...
//            JasperPrint print = JasperFillManager.fillReport("D:/Project/lc_export_import/src/main/resources/reports" + File.separator + jasperName + ".jasper", reportParams/*, dataSource.getConnection()*/, con);
    ...
    @Override
    public <B extends BaseModel, T> List<T> getDtoList(List<B> bList, Class<T> targetClass) {
        return bList.stream().map(b -> modelMapper.map(b, targetClass)).collect(Collectors.toList());
    }
